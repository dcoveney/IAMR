...
!c ::: -----------------------------------------------------------
      ...
      REAL_T    dx(SDIM), xlo(SDIM), problo(SDIM), time, tdx, tdy
      integer   tag(DIMV(tag))
      REAL_T    rho(DIMV(rho), nvar), rhograd(DIMV(rho), nvar), maxrhograd
	  REAL_T 	drdx, drdy	
      integer   i, j, k
...
#include <probdata.H>
 ...
 !    do       k = lo(3), hi(3)
      do j = lo(2), hi(2)
         do i = lo(1), hi(1)
            drdx = (rho(i+1,j,1)-rho(i-1,j,1))/tdx
            drdy = (rho(i,j+1,1)-rho(i,j-1,1))/tdy
            rhograd(i,j,1) = sqrt( drdx*drdx + drdy*drdy )
         end do
      end do
	maxrhograd = maxval(rhograd)
  !    end do
	write(*,*) maxrhograd

      do j = lo(2), hi(2)
         do i = lo(1), hi(1)
!			if(rhograd(i,j,1) > 0.2*maxrhograd) then
!			 tag(i,j) = set
!			end if
            tag(i,j) = merge(set,tag(i,j),rhograd(i,j,1).gt.0.4*maxrhograd)
         end do
      end do
