         ...
         do i = ARG_L1(rho), domlo(1)-1
            do j = ARG_L2(rho), ARG_H2(rho)
                ...
                if(y.le.0.925 .and. y.gt.0.90) then
					rho(i,j) = 897.0d0
				else if(y.le.0.90 .and. y.ge.0.875) then
					rho(i,j) = 999.2d0
				else
...
!	  if(probtype.eq.20 .and.ARG_H1(rho).gt.domhi(1)) then
!		 do i = domhi(1), ARG_H1(rho)
!           do j = ARG_L2(rho), ARG_H2(rho)
!                y = xlo(2) + dx(2)*(float(j-lo(2)) + half)
      ...
      if (bc(1,1).eq.EXT_DIR.and.ARG_L1(adv).lt.domlo(1)) then
         do i = ARG_L1(adv), domlo(1)-1
            do j = ARG_L2(adv), ARG_H2(adv)
				if(probtype .eq. 22) then
                y = xlo(2) + dx(2)*(float(j-lo(2)) + half)
                if(y.le.0.90 .and. y.ge.0.875) then
					adv(i,j) = one
				else
...
!c ::: -----------------------------------------------------------
      ...
      integer    domlo(SDIM), domhi(SDIM)
      REAL_T     dx(SDIM), xlo(SDIM), time
      REAL_T     u(DIMV(u)), x_vel, y, tg, got
...
!	  if(probtype.eq.20 .and.ARG_H1(u).gt.domhi(1)) then

!		 do i = domhi(1), ARG_H1(u)
...
!	 end if
      ...
      if (bc(1,1).eq.EXT_DIR.and.ARG_L1(u).lt.domlo(1)) then
         do i = ARG_L1(u), domlo(1)-1
			...
			if(probtype .eq. 22) then
                y = xlo(2) + dx(2)*(float(j-lo(2)) + half)
                if(y.le.0.925 .and. y.ge.0.875) then
					u(i,j) = 2.0
				else
					u(i,j) = zero
				end if
