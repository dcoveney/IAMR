...
!                              vel, v_lo, v_hi,&
!                              scal, s_lo, s_hi,&
!                              dx, xlo, xhi, gravity, sigma, scomp, ncomp, &
...
!      REAL_T, dimension(v_lo(1):v_hi(1),v_lo(2):v_hi(2),v_lo(3):v_hi(3),0:SDIM-1) :: vel
!      REAL_T, dimension(s_lo(1):s_hi(1),s_lo(2):s_hi(2),s_lo(3):s_hi(3),0:nscal-1) :: scal
!      REAL_T, dimension(sgrad_lo(1):sgrad_hi(1),sgrad_lo(2):sgrad_hi(2),sgrad_lo(3):sgrad_hi(3),0:2) :: scalgrad
...
!      REAL_T  :: scalmin(0:nscal-1)
!      REAL_T  :: scalmax(0:nscal-1)
!      REAL_T  :: scalgradmin(0:2)
!      REAL_T  :: scalgradmax(0:2)
!      REAL_T  :: forcemin(scomp:scomp+ncomp-1)
!      REAL_T  :: forcemax(scomp:scomp+ncomp-1)
!      REAL_T  :: x, y, z
!      REAL_T  :: hx, hy, hz
!      REAL_T  :: sga, cga, Fsvx, Fsvy
...
!      REAL_T, dimension(s_lo(1):s_hi(1)+2,s_lo(2):s_hi(2)+2,s_lo(3):s_hi(3)+2) :: scalExt
!      REAL_T, dimension(s_lo(1):s_hi(1)+2,s_lo(2):s_hi(2)+2,s_lo(3):s_hi(3)+2, 0:1) :: normExt
!!      REAL_T, dimension(f_lo(1):f_hi(1),f_lo(2):f_hi(2),f_lo(3):f_hi(3), 0:1) :: scalgrad
!      REAL_T, dimension(s_lo(1):s_hi(1),s_lo(2):s_hi(2),s_lo(3):s_hi(3), 0:1) :: scalnorm
!      REAL_T, dimension(f_lo(1):f_hi(1),f_lo(2):f_hi(2),f_lo(3):f_hi(3)) :: curv
!      integer :: kx, ky, mode_count, xstep, ystep
!      integer :: isioproc
!      integer :: nXvel, nYvel, nZvel, nRho, nTrac, nTrac2
!      integer :: nRhoScal, nTracScal, nTrac2Scal
...
!            enddo
!            do n = 0, 2
!               scalgradmin(n) = 1.d234
!               scalgradmax(n) = -1.d234
!            enddo
...
!                     enddo
!                     do n = 0, 2
!                        if (scalgrad(i,j,k,n)>scalgradmax(n)) then
!                           scalgradmax(n)=scalgrad(i,j,k,n)
!                        endif
!                        if (scalgrad(i,j,k,n)<scalgradmin(n)) then
!                           scalgradmin(n)=scalgrad(i,j,k,n)
!                        endif
!                     enddo
...
!            enddo
!            do n = 0, 2
!               write (6,*) "scalgradmin(",n,") = ",scalgradmin(n)
!               write (6,*) "scalgradmax(",n,") = ",scalgradmax(n)
!            enddo
!         endif
!      endif
!     do k = s_lo(3), s_hi(3)
!         do j = s_lo(2), s_hi(2)
!           do i = s_lo(1), s_hi(1)
!				scalExt(i+1,j+1,k) = scal(i,j,k,nTracScal) 
!           end do
!         end do
!      end do
...
!!      do k = s_lo(3), s_hi(3)
!!         do j = s_lo(2), s_hi(2)
!!			scalExt(s_lo(1),j+1,k) = scal(s_lo(1),j,k,nTracScal)
!!			scalExt(s_hi(1)+2,j+1,k) = scal(s_hi(1),j,k,nTracScal)
!!         end do
!!      end do

!!      do k = s_lo(3), s_hi(3)
!!         do i = s_lo(1), s_hi(1)
!!			scalExt(i+1,s_lo(2),k) = scal(i,s_lo(2),k,nTracScal)
!!			scalExt(i+1,s_hi(2)+2,k) = scal(i,s_hi(2),k,nTracScal)
!!         end do
!!      end do
...
!!           if(i .eq. s_lo(1)) then
!!			dTdx = ((scal(i+1,j,k,nTracScal)-scal(i,j,k,nTracScal)))/hx
!!		   else if(i .eq. s_hi(1)) then
!!			dTdx = ((scal(i,j,k,nTracScal)-scal(i-1,j,k,nTracScal)))/hx
!!		   else
!!			dTdx = (scal(i+1,j,k,nTracScal)-scal(i-1,j,k,nTracScal))/(2*hx)
!!		   end if
!!           if(j .eq. s_lo(2)) then
!!			dTdy = ((scal(i,j+1,k,nTracScal)-scal(i,j,k,nTracScal)))/hy
!!		   else if(i .eq. s_hi(1)) then
!!			dTdy = ((scal(i,j,k,nTracScal)-scal(i,j-1,k,nTracScal)))/hy
!!		   else
!!			dTdy = (scal(i,j+1,k,nTracScal)-scal(i,j-1,k,nTracScal))/(2*hy)
!!		   end if			
           
!!		   dTdx = (scalExt(i+2,j+2,k) + 2.0*scalExt(i+2,j+1,k) + scalExt(i+2, j+1, k) &
!!		   & - scalExt(i,j+2,k) - 2.0*scalExt(i,j+1,k) - scalExt(i,j,k))/(2.0*hx)
!!		   dTdy = (scalExt(i+2,j+2,k) + 2.0*scalExt(i+1,j+2,k) + scalExt(i, j+2, k) &
!!		   & - scalExt(i+2,j,k) - 2.0*scalExt(i+1,j,k) - scalExt(i,j,k))/(2.0*hy)
		   
!		   dTdx = (scal(i+1,j,k,nTracScal)-scal(i-1,j,k,nTracScal))/(two*hx)
!		   dTdy = (scal(i,j+1,k,nTracScal)-scal(i,j-1,k,nTracScal))/(two*hy)
!!		   dTdx = (scalExt(i+2,j+1,k)-scalExt(i,j+1,k))/(2*hx)
!!!		   dTdy = (scalExt(i+1,j+2,k)-scalExt(i+1,j,k))/(2*hy)
		   
!!           if(vel(i,j,k,nTracScal) .ge. 0) then
!!		   dTdx = (scal(i,j,k,nTracScal)-scal(i-1,j,k,nTracScal))/(hx)
...
!           d2Tdx2 = (scal(i+1,j,k,nTracScal)-2*scal(i,j,k,nTracScal)+scal(i-1,j,k,nTracScal))/(hx**2)
!           d2Tdy2 = (scal(i,j+1,k,nTracScal)-2*scal(i,j,k,nTracScal)+scal(i,j-1,k,nTracScal))/(hy**2)
!           d2Tdxdy = (scal(i+1,j+1,k,nTracScal)-scal(i+1,j-1,k,nTracScal)-scal(i-1,j+1,k,nTracScal)+scal(i-1,j-1,k,nTracScal))/(4*hy*hx)
!!           d2Tdydx = (scal(i-1,j+2,k,nTracScal)-scal(i-1,j+2,k,nTracScal)-scal(i-1,j,k,nTracScal)+scal(i-1,j,k,nTracScal))/(4*hy*hx)
!           !d2Tdydx = (scalExt(i+2, j+2, k) - scal(i+1, j+2, k)
           
!           dTdxPx = (scal(i+1,j,k,nTracScal)-scal(i,j,k,nTracScal))/(hx) ! nx_{i+1/2,j}
!           dTdxMx = (scal(i,j,k,nTracScal)-scal(i-1,j,k,nTracScal))/(hx) ! nx_{i-1/2,j}
!           dTdxPxPy = (scal(i+1,j+1,k,nTracScal)-scal(i,j+1,k,nTracScal))/(hx) ! nx_{i+1/2,j+1}
!           dTdxMxPy = (scal(i,j+1,k,nTracScal)-scal(i-1,j+1,k,nTracScal))/(hx) ! nx_{i-1/2,j+1}
!           dTdxPxMy = (scal(i+1,j-1,k,nTracScal)-scal(i,j-1,k,nTracScal))/(hx) ! nx_{i+1/2,j-1}
!           dTdxMxMy = (scal(i,j-1,k,nTracScal)-scal(i-1,j-1,k,nTracScal))/(hx) ! nx_{i-1/2,j-1}
           
!           dTdyPy = (scal(i,j+1,k,nTracScal)-scal(i,j,k,nTracScal))/(hy) ! ny_{i,j+1/2}
!           dTdyMy = (scal(i,j,k,nTracScal)-scal(i,j-1,k,nTracScal))/(hy) ! ny_{i,j-1/2}
!           dTdyPyPx = (scal(i+1,j+1,k,nTracScal)-scal(i+1,j,k,nTracScal))/(hy) ! ny_{i+1/2,j+1/2}
!           dTdyMyPx = (scal(i+1,j,k,nTracScal)-scal(i+1,j-1,k,nTracScal))/(hy) ! ny_{i+1/2, j-1/2}
!           dTdyPyMx = (scal(i-1,j+1,k,nTracScal)-scal(i-1,j,k,nTracScal))/(hy) ! ny_{i-1/2, j+1/2}
!           dTdyMyMx = (scal(i-1,j-1,k,nTracScal)-scal(i-1,j-1,k,nTracScal))/(hy) ! ny_{i-1/2, j-1/2}
           
!           dTdxPhalfy = 0.25*(dTdxPx + dTdxPxPy+ dTdxMx + dTdxMxPy) ! n_x{i, j+1/2}
!           dTdyPhalfx = 0.25*(dTdyPy + dTdyPyPx + dTdyMy + dTdyMyPx) ! n_y{i+1/2,j}
!           dTdxMhalfy = 0.25*(dTdxPxMy + dTdxPx + dTdxMxMy + dTdxMx) ! n_x{i, j-1/2}
!           dTdyMhalfx = 0.25*(dTdyPyMx + dTdyPy + dTdyMyMx + dTdyMyPx) ! n_y{i-1/2, j}
           
!           magNPx = DSQRT(dTdxPx**2 + dTdyPhalfx**2) ! |n_{i+1/2,j}|
!           magNMx = DSQRT(dTdxMx**2 + dTdyMhalfx**2) ! |n_{i-1/2,j}|
!           magNPy = DSQRT(dTdxPhalfy**2 + dTdyPy**2) ! |n_{i,j+1/2}|
!           magNMy = DSQRT(dTdxMhalfy**2 + dTdyMy**2) ! |n_{i,j-1/2}|
...
!		   gradN = (1/hx)*((dTdxPx/magNPx)-(dTdxMx/magNMx)) + (1/hy)*((dTdyPy/magNPy) - (dTdyMy/magNMy))
			
!		   curv(i,j,k) = 1.0/0.02	
!           !curv(i,j,k) = gradN

!		   scalgrad(i,j,k,0) = dTdx
!		   scalgrad(i,j,k,1) = dTdy
				
!!           if(vel(i,j,k,0) .ge. 0) then
!!			scalgrad(i,j,k,0) = (scal(i+1,j,k, nTracScal)-scal(i,j,k, nTracScal))/(hx)

!!		  else 
!!			scalgrad(i,j,k,0) = (scal(i,j,k, nTracScal)-scal(i-1,j,k, nTracScal))/(hx)
!!		   end if
		   
!!		   if(vel(i,j,k,1) .ge. 0) then
!!			scalgrad(i,j,k,1) = (scal(i,j+1,k, nTracScal)-scal(i,j,k, nTracScal))/(hy)
!!		   else 
!!			scalgrad(i,j,k,1) = (scal(i,j,k, nTracScal)-scal(i,j-1,k, nTracScal))/(hy)
!!		   end if
!           magn = (dTdx**2 + dTdy**2)
!           if((magn .eq. 0) .or. ((dTdx+dTdy) .eq. 0)) then
!			magn = 1.0d0
!			end if
!   !        scalnorm(i,j,k,0) = dTdx/magn
!    !       scalnorm(i,j,k,1) = dTdy/magn
!!		   if(isnan(curv(i,j,k))) then
!!			write(*,*) magNPx
!!			stop 'norm is nan' 
!!		   end if
!!		   curv(i,j,k) = (dTdx*(dTdy*d2Tdx2 - dTdx*d2Tdxdy) - dTdx*(dTdy*d2Tdydx - dTdx*d2Tdy2))/(magn**(1.5))
!		   !curv(i,j,k) = ((dTdx**2)*d2Tdy2 - 2*dTdx*dTdy*d2Tdxdy + (dTdy**2)*d2Tdx2)/(magn**(1.5))
!           end do
!         end do
!      end do
...
!!         do j = s_lo(2), s_hi(2)
!!           do i = s_lo(1), s_hi(1)
!!				normExt(i+1,j+1,k,0) = scalnorm(i,j,k,0) 
!!				normExt(i+1,j+1,k,1) = scalnorm(i,j,k,1) 
!!           end do
!!         end do
!!      end do
...
!!      do k = s_lo(3), s_hi(3)
!!         do j = s_lo(2), s_hi(2)
!!			normExt(s_lo(1),j+1,k,0) = scalnorm(s_lo(1),j,k,0)
!!			normExt(s_lo(1),j+1,k,1) = scalnorm(s_lo(1),j,k,1)
			
!!			normExt(s_hi(1)+2,j+1,k,0) = normExt(s_hi(1)+1,j+1,k,0)
!!			normExt(s_hi(1)+2,j+1,k,1) = normExt(s_hi(1)+1,j+1,k,1)
!!         end do
!!      end do

!!      do k = s_lo(3), s_hi(3)
!!         do i = s_lo(1), s_hi(1)
!!			normExt(i+1,s_lo(2),k,0) = scalnorm(i,s_lo(2),k,0)
!!			normExt(i+1,s_lo(2),k,1) = scalnorm(i,s_lo(2),k,1)
			
!!			normExt(i+1,s_hi(2)+2,k,0) = normExt(i+1,s_hi(2)+1,k,0)
!!			normExt(i+1,s_hi(2)+2,k,1) = normExt(i+1,s_hi(2)+1,k,1)
!!!			if(i .eq. s_hi(1)) then
!!!				write(*,*) scalnorm(i,s_lo(2),k,0)
!!!			end if
!!         end do
!!      end do
...
!!           do i = s_lo(1), s_hi(1)
!!           if(i .eq. s_lo(1)) then
!!			dNxdx = ((scalnorm(i+1,j,k,0)-scalnorm(i,j,k,0)))/hx
!!		   else if(i .eq. s_hi(1)) then
!!			dNxdx = ((scalnorm(i,j,k,0)-scalnorm(i-1,j,k,0)))/hx
!!		   else
!!			dNxdx = (scalnorm(i+1,j,k,0)-scalnorm(i-1,j,k,0))/(2*hx)
!!		   end if
!!           if(j .eq. s_lo(2)) then
!!			dNydy = ((scalnorm(i,j+1,k,0)-scalnorm(i,j,k,0)))/hy
!!		   else if(i .eq. s_hi(1)) then
!!			dNydy = ((scalnorm(i,j,k,0)-scalnorm(i,j-1,k,0)))/hy
!!		   else
!!			dNydy = (scalnorm(i,j+1,k,0)-scalnorm(i,j-1,k,0))/(2*hy)
!!		   end if			

!!			curv(i,j,k) = -one*(dNxdx+dNydy) 
!!			if(isnan(curv(i,j,k))) then
!!			write(*,*) i
!!			write(*,*) j
!!			write(*,*) s_hi(1)
...
!               do j = f_lo(2), f_hi(2)
!                  do i = f_lo(1), f_hi(1)
!					 Fsvx = 2.0*sigma*curv(i,j,k)*scalgrad(i,j,k,0)*scal(i,j,k,nTracScal)
!					 Fsvy = 2.0*sigma*curv(i,j,k)*scalgrad(i,j,k,1)*scal(i,j,k,nTracScal)
!                     force(i,j,k,nXvel) = scal(i,j,k,nRhoScal)*sga + Fsvx
!#if ( AMREX_SPACEDIM == 2 )
!                     force(i,j,k,nYvel) = scal(i,j,k,nRhoScal)*cga + Fsvy
!#elif ( AMREX_SPACEDIM == 3 )
!                     force(i,j,k,nYvel) = zero
!                     force(i,j,k,nZvel) = scal(i,j,k,nRhoScal)*cga
!#endif
...
!               do j = f_lo(2), f_hi(2)
!                  do i = f_lo(1), f_hi(1)
!                  	 Fsvx = 2.0*sigma*curv(i,j,k)*scal(i,j,k,nTracScal)*scalgrad(i,j,k,0)
!					 Fsvy = 2.0*sigma*curv(i,j,k)*scal(i,j,k,nTracScal)*scalgrad(i,j,k,1)
!                     force(i,j,k,nXvel) = zero + Fsvx
!#if ( AMREX_SPACEDIM == 2 )
!                     force(i,j,k,nYvel) = gravity*scal(i,j,k,nRhoScal) + Fsvy
!#elif ( AMREX_SPACEDIM == 3 )
!                     force(i,j,k,nYvel) = zero
!                     force(i,j,k,nZvel) = gravity*scal(i,j,k,nRhoScal)
!#endif
...
!               do j = f_lo(2), f_hi(2)
!                  do i = f_lo(1), f_hi(1)
!                  	 Fsvx = 2.0*sigma*scalgrad(i,j,k,2)*scal(i,j,k,nTracScal)*scalgrad(i,j,k,0)
!					 Fsvy = 2.0*sigma*scalgrad(i,j,k,2)*scal(i,j,k,nTracScal)*scalgrad(i,j,k,1)
!                     force(i,j,k,nXvel) = zero + Fsvx
!                     force(i,j,k,nYvel) = zero + Fsvy
!#if ( AMREX_SPACEDIM == 3 )
!                     force(i,j,k,nZvel) = zero
!#endif
!                  enddo
...
!c     probtype = RT
        ...
        do j = lo(2), hi(2)
           do i = lo(1), hi(1)
              tag(i,j) = merge(set,tag(i,j),adv(i,j,1).le.0.5)
           end do
        end do
...
!c ::: -----------------------------------------------------------
      ...
      integer    bc(SDIM,2)
      integer    lo(SDIM),hi(SDIM)
      REAL_T 	 ampl, omega, g, rho_1, rho_2, rho_m, u_1, u_2, u_m
      REAL_T 	 l, l_p, d, y
      integer    i, j
...
#include <probdata.H>
	  rho_1 = 1.0d0
      rho_2 = 7.0d0
	  rho_m = (rho_1+rho_2)/2.d0
	  l_p = (rho_1-rho_2)/(rho_1+rho_2)
	  u_1 = 1.45d0
	  u_2 = 0.55d0
	  l = (u_1-u_2)/(u_1+u_2)
	  u_m = (u_1+u_2)/2.d0
         ...
         do i = ARG_L1(rho), domlo(1)-1
            do j = ARG_L2(rho), ARG_H2(rho)
				...
				if(probtype.eq.15) then
					rho(i,j) = rho_m*(1+l_p*tanh(2*y/d))
...
!c ::: -----------------------------------------------------------
      ...
      integer    DIMDEC(u)
      integer    domlo(SDIM), domhi(SDIM), seed 
      REAL_T     dx(SDIM), xlo(SDIM), time
      REAL_T     u(DIMV(u)), x_vel
      integer    lo(SDIM),hi(SDIM), bc(SDIM,2), i, j
      REAL_T 	 ampl(10), omega(10), g, rho_1, rho_2, rho_m, u_1, u_2, u_m
      REAL_T 	 l, l_p, d, x, y, sumHarmonic, pert, c, alpha
...
#include <INFL_FORCE_F.H>
      ...
      ampl(1) = 0.01d0
      omega(1) = 0.438d0
      omega(2) = 0.75d0*omega(1)
      omega(3) = 0.55d0*omega(1)
      omega(4) = 0.44d0*omega(1)
      do i = 2, 10
		ampl(i) = ampl(1)/i
	  end do
	  
	  do i = 5, 10
			omega(i) = 1.7*omega(1)/i
	  end do
	  
	  do i = 1, 10
		sumHarmonic = sumHarmonic + ampl(i)*sin(omega(i)*time)
	  end do
...
!		  u(i,j) = cos(alpha*(x-c*time))*alpha*(exp(alpha*y)-exp(-one*alpha*y))		
!		end do
!	  end do
!      end if
      ...
      if (bc(1,1).eq.EXT_DIR.and.ARG_L1(u).lt.domlo(1)) then
         do i = ARG_L1(u), domlo(1)-1
            do j = ARG_L2(u), ARG_H2(u)
                y = xlo(2) + dx(2)*(float(j-lo(2)) + half)
				if(probtype.eq.15) then
					u(i,j) = u_m*(1+l*tanh(2*y/d))*g
				else if(probtype.eq.16) then
					u(i,j) = u_m*(1+l*tanh(2*y/d))
				else
	               u(i,j) = x_vel
	            end if
            end do
...
!          y = xlo(2) + dx(2)*(float(j-lo(2)) + half)
!          x = xlo(1) + dx(1)*(float(i-lo(1)) + half)
!		  v(i,j) = alpha*sin(alpha*(x-c*time))*(exp(alpha*y)+exp(-one*alpha*y))		
!		end do
!	  end do
!      end if
!	call random_seed(seed)
